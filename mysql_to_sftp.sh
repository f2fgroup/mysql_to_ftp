...
# SQL INSERT export settings
SQL_EXPORT_ENABLED="${SQL_EXPORT_ENABLED:-false}"
SQL_OUTPUT_DIR="${SQL_OUTPUT_DIR:-${OUTPUT_DIR}}"
SFTP_SQL_REMOTE_DIR="${SFTP_SQL_REMOTE_DIR:-${SFTP_REMOTE_DIR}}"
...
_AWK_SQL='
    ...
    NR==1 {
        ...
        ncols=NF
        printf "-- SQL export generated by mysql_to_sftp.sh\n-- Table: %s\nSET NAMES utf8mb4;\n\n", TABLENAME
        next
    }
    {
...
# Function to execute MySQL query and generate a SQL INSERT file locally
execute_query_to_sql() {
    local sql_query="$1"
    local output_file="$2"
    local table_name="$3"

    log "Generating SQL export: $output_file"
    rm -f "$output_file" 2>/dev/null || true
    ...
    if [[ -n "$MYSQL_PASSWORD" ]]; then
        ...
        if mysql --defaults-extra-file="$tmp_defaults" \
            ...
            --batch \
            -e "${sql_query}" 2>"$tmp_mysql_stderr" | \
            awk -v TABLENAME="${table_name}" \
                "$_AWK_SQL" > "$output_file"; then
            mysql_status=0
        fi
    ...
    if [[ $mysql_status -ne 0 ]]; then
        rm -f "$output_file"
        if [[ -s "$tmp_mysql_stderr" ]]; then
            log_error "MySQL error (SQL export): $(cat "$tmp_mysql_stderr")"
        fi
        rm -f "$tmp_mysql_stderr"
        log_error "Failed to execute query for SQL export"
        return 1
    fi
    ...
    if [[ ! -s "$output_file" ]]; then
        rm -f "$output_file"
        rm -f "$tmp_mysql_stderr"
        log_error "SQL export file is empty: $output_file"
        return 1
    fi

    rm -f "$tmp_mysql_stderr"
    log "Successfully generated SQL file: $output_file"
    return 0
}

# Function to upload file to SFTP using atomic operation (put to .part, then rename)
# $4 is an optional remote directory (defaults to SFTP_REMOTE_DIR)
upload_to_sftp() {
    local local_file="$1"
    local remote_filename="$2"
    local hostkey_opts="$3"
    local remote_dir="${4:-${SFTP_REMOTE_DIR}}"

    local remote_temp="${remote_filename}.part"
    local remote_final="${remote_dir}/${remote_filename}"

    log "Uploading file to SFTP: $local_file -> $remote_final"

    local sftp_batch_content
    sftp_batch_content=$(printf 'cd "%s"\nput "%s" "%s"\nrename "%s" "%s"\nbye\n' \
        "$remote_dir" "$local_file" "$remote_temp" "$remote_temp" "$remote_filename")
...
process_sql_file() {
    ...
    if [[ "${SQL_EXPORT_ENABLED}" == "true" ]]; then
        local sql_output_file="${SQL_OUTPUT_DIR}/${basename}.sql"
        if execute_query_to_sql "$query" "$sql_output_file" "$basename"; then
            if ! upload_to_sftp "$sql_output_file" "${basename}.sql" "$hostkey_opts" "$SFTP_SQL_REMOTE_DIR"; then
                log_error "Failed to upload SQL export for: $sql_file"
                return 1
            fi
            # Uncomment to remove local SQL files after upload:
            # rm -f "$sql_output_file"
        else
            log_error "Failed to generate SQL export for: $sql_file"
            return 1
        fi
    fi
...
main() {
    ...
    # If SQL export is enabled and uses a distinct remote directory, ensure it exists
    if [[ "${SQL_EXPORT_ENABLED}" == "true" && "${SFTP_SQL_REMOTE_DIR}" != "${SFTP_REMOTE_DIR}" ]]; then
        if ! run_sftp_batch "$hostkey_opts" "$(printf 'ls "%s"\nbye' "${SFTP_SQL_REMOTE_DIR}")" "true"; then
            run_sftp_batch "$hostkey_opts" "$(printf 'mkdir "%s"\nbye' "${SFTP_SQL_REMOTE_DIR}")" "true" || true
        fi
        log "SQL export remote directory ready: ${SFTP_SQL_REMOTE_DIR}"
    fi
